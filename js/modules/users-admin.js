...
class UsersAdminModule {
    ...
    bindEvents() {
        ...
        ['filter-users-search', 'filter-users-category', 'filter-users-building', 'filter-users-sort-order', 'filter-users-page-size'].forEach(id => {
        ...
        const pillAll = document.getElementById('pill-users-all');
        if (pillAll) pillAll.addEventListener('click', () => this.resetFilters());

        const pillYear = document.getElementById('pill-users-this-year');
        if (pillYear) pillYear.addEventListener('click', () => this.applyFilters());

        const pillMonth = document.getElementById('pill-users-this-month');
        if (pillMonth) pillMonth.addEventListener('click', () => this.applyFilters());

        const pillToday = document.getElementById('pill-users-today');
        if (pillToday) pillToday.addEventListener('click', () => this.applyFilters());
    }

    resetFilters() {
        const s = document.getElementById('filter-users-search'); if (s) s.value = '';
        const su = document.getElementById('search-users'); if (su) su.value = '';
        const c = document.getElementById('filter-users-category'); if (c) c.value = 'all';
        const b = document.getElementById('filter-users-building'); if (b) b.value = 'all';
        this.searchKeyword = '';
        this.applyFilters();
    }
    ...
    applyFilters() {
        let list = [...this.items];
        const searchInputVal = document.getElementById('filter-users-search')?.value || document.getElementById('search-users')?.value || '';
        const searchVal = (this.searchKeyword || searchInputVal).trim().toLowerCase();
        const roleVal = document.getElementById('filter-users-category')?.value || 'all';
        const sortOrder = document.getElementById('filter-users-sort-order')?.value || '';
        const pageSize = document.getElementById('filter-users-page-size')?.value || 'all';
        ...
        if (roleVal !== 'all' && roleVal) {
            list = list.filter(item => (item.role && item.role.toLowerCase().includes(roleVal.toLowerCase())));
        }

        if (sortOrder === 'name_asc') {
            this.sortField = 'username'; this.sortDir = 'asc';
        } else if (sortOrder === 'name_desc') {
            this.sortField = 'username'; this.sortDir = 'desc';
        }
        ...
        const badge = document.getElementById('users-total-count-badge');
