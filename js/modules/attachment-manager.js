...
class AttachmentManager {
    ...
    normalizeAttachments(raw) {
        ...
        if (Array.isArray(raw)) {
            ...
            if (trimmed.startsWith('[') && trimmed.endsWith(']')) {
                try {
                    ...
                    // Fallback: strip outer brackets and quotes if JSON parse fails
        ...
        return list.map(item => {
            ...
            if (typeof item === 'string') {
                let str = item.trim();
                // Clean up any stray quotes or brackets if present
    ...
    openMediaViewer(item) {
        ...
        const isWord = (typeof item === 'object' && item.type === 'word') || dataStr.includes('.docx') || dataStr.includes('.doc') || dataStr.includes('wordprocessingml') || dataStr.includes('msword');
        const isExcel = (typeof item === 'object' && item.type === 'excel') || dataStr.includes('.xlsx') || dataStr.includes('.xls') || dataStr.includes('spreadsheetml') || dataStr.includes('vnd.ms-excel');

        if (isWord || isExcel) {
            const isXls = isExcel;
            const iconClass = isXls ? 'fa-solid fa-file-excel text-emerald-400' : 'fa-solid fa-file-word text-blue-400';
            const typeLabel = isXls ? 'เอกสาร Microsoft Excel' : 'เอกสาร Microsoft Word';
            const btnColor = isXls ? '#10b981' : '#3b82f6';
            const ext = isXls ? '.xlsx' : '.docx';

            Swal.fire({
                title: `<div class="text-base font-bold text-white flex items-center justify-center gap-2"><i class="${iconClass}"></i> ${name}</div>`,
                html: `
                    <div class="p-6 text-center bg-slate-900 rounded-xl border border-slate-700 my-2">
                        <div class="mb-3"><i class="${iconClass} text-5xl"></i></div>
                        <p class="text-sm text-slate-200 font-bold mb-1">${typeLabel}</p>
                        <p class="text-xs text-slate-400">กรุณากดปุ่มด้านล่างเพื่อดาวน์โหลดและเปิดดูเอกสารด้วยโปรแกรม Office บนอุปกรณ์ของคุณ</p>
                    </div>
                `,
                showCancelButton: true,
                cancelButtonText: 'ปิด',
                confirmButtonText: '<i class="fa-solid fa-download mr-1"></i> ดาวน์โหลดไฟล์',
                confirmButtonColor: btnColor,
                preConfirm: () => {
                    try {
                        const link = document.createElement('a');
                        link.href = data;
                        const safeName = name ? (name.includes('.') ? name : name + ext) : ('เอกสาร' + ext);
    ...
    /**
     * =========================================================================
     * UNIVERSAL DROPZONE BINDER (รองรับ PDF, รูปภาพ, Word, Excel, URL, Base64)
     * =========================================================================
     */
    detectFileTypeMeta(fileNameOrUrl, mimeType = '') {
        const str = String(fileNameOrUrl || '').toLowerCase();
        const mime = String(mimeType || '').toLowerCase();

        if (str.endsWith('.pdf') || str.includes('application/pdf') || mime.includes('pdf')) {
            return {
                type: 'pdf',
                label: 'PDF',
        ...
        if (str.endsWith('.docx') || str.endsWith('.doc') || mime.includes('word') || mime.includes('msword') || mime.includes('wordprocessingml')) {
            return {
                type: 'word',
                label: 'Word',
        ...
        if (str.endsWith('.xlsx') || str.endsWith('.xls') || mime.includes('excel') || mime.includes('spreadsheet') || str.endsWith('.csv')) {
            return {
                type: 'excel',
                label: 'Excel',
        ...
        if (str.match(/\.(jpg|jpeg|png|webp|gif|bmp|svg)$/) || mime.startsWith('image/') || str.startsWith('data:image/')) {
            return {
                type: 'image',
                label: 'รูปภาพ',
    ...
    bindUniversalDropzone(prefix, options = {}) {
        window._universalDropzones = window._universalDropzones || {};

        const dropzone = document.getElementById(`${prefix}-dropzone`);
        const fileInput = document.getElementById(`${prefix}-file-upload-input`);
        const promptEl = document.getElementById(`${prefix}-dropzone-prompt`);
        const infoEl = document.getElementById(`${prefix}-dropzone-fileinfo`);
        const nameEl = document.getElementById(`${prefix}-dropzone-filename`) || document.getElementById(`${prefix}-file-name`);
        const sizeEl = document.getElementById(`${prefix}-dropzone-filesize`) || document.getElementById(`${prefix}-file-meta`);
        const iconEl = document.getElementById(`${prefix}-file-type-icon`) || document.getElementById(`${prefix}-file-icon`);
        const removeBtn = document.getElementById(`${prefix}-btn-remove-file`) || document.getElementById(`${prefix}-btn-clear-file`);
        const urlInput = document.getElementById(`${prefix}-form-url`) || 
                         document.getElementById(`${prefix}-form-image`) || 
                         document.getElementById(`${prefix}-form-file`) ||
                         (prefix === 'user' ? document.getElementById('uadm-form-image') : null);
        ...
        window._universalDropzones[prefix] = config;

        // Click to browse
        dropzone.addEventListener('click', (e) => {
            if (e.target.closest(`#${prefix}-btn-remove-file`) || e.target.closest('button')) return;
            if (e.target.closest(`#${prefix}-dropzone-fileinfo`) && config.currentFile) {
                // If clicking on file info card, view the file
                this.viewCurrentDropzoneFile(prefix);
                return;
            }
        ...
        ['dragenter', 'dragover'].forEach(evt => {
            dropzone.addEventListener(evt, (e) => {
                e.preventDefault();
                e.stopPropagation();
                dropzone.classList.add('border-emerald-400', 'bg-slate-900/95');
            });
        });
        ...
        ['dragleave', 'drop'].forEach(evt => {
            dropzone.addEventListener(evt, (e) => {
                e.preventDefault();
                e.stopPropagation();
                dropzone.classList.remove('border-emerald-400', 'bg-slate-900/95');
            });
        });
        ...
        dropzone.addEventListener('drop', async (e) => {
            ...
            if (files && files.length > 0) {
                await this.handleDropzoneFileInput(prefix, files[0]);
            }
        });
        ...
        if (urlInput) {
            urlInput.addEventListener('change', () => {
                ...
                if (val) {
                    this.setDropzoneValue(prefix, val, 'ลิงก์ภายนอก');
                } else {
                    this.resetDropzone(prefix, false);
                }
            });
        }
    }

    async handleDropzoneFileInput(prefix, file) {
        const config = window._universalDropzones && window._universalDropzones[prefix];
        ...
        try {
            ...
            Swal.close();
            if (processed) {
                config.currentFile = processed;
                this.updateDropzoneUI(prefix, processed.name, processed.size, processed.data);
                if (config.urlInput) {
                    config.urlInput.value = processed.data;
                }
                if (typeof config.onFileChange === 'function') {
                    config.onFileChange(processed);
                }
            }
        } catch (err) {
            Swal.close();
            console.error('Dropzone file error:', err);
            Swal.fire({ icon: 'error', title: 'อัปโหลดไม่สำเร็จ', text: err.message });
        }
    }

    updateDropzoneUI(prefix, fileName, fileSizeStr, dataUrl) {
        const config = window._universalDropzones && window._universalDropzones[prefix];
        if (!config) return;

        const meta = this.detectFileTypeMeta(fileName || dataUrl);

        if (config.promptEl) config.promptEl.classList.add('hidden');
        if (config.infoEl) {
            config.infoEl.classList.remove('hidden');
            config.infoEl.classList.add('flex');
        }
        if (config.nameEl) {
            config.nameEl.textContent = fileName || 'ไฟล์แนบ';
            config.nameEl.title = fileName || 'ไฟล์แนบ';
        }
        if (config.sizeEl) {
            config.sizeEl.textContent = `${meta.label} • ${fileSizeStr || 'พร้อมใช้งาน'}`;
        }
        if (config.iconEl) {
            if (meta.type === 'image' && dataUrl) {
                config.iconEl.className = 'w-12 h-12 rounded-lg overflow-hidden border border-amber-500/50 shrink-0 bg-slate-950 flex items-center justify-center cursor-pointer shadow-md hover:scale-105 transition-transform';
                config.iconEl.innerHTML = `<img src="${dataUrl}" class="w-full h-full object-cover" alt="รูปตัวอย่าง" title="คลิกดูภาพขยาย" />`;
            } else {
                config.iconEl.className = `w-10 h-10 rounded-lg ${meta.bgClass} border ${meta.borderClass} flex items-center justify-center text-lg ${meta.colorClass} shrink-0`;
                config.iconEl.innerHTML = `<i class="${meta.icon}"></i>`;
            }
        }

        // Auto-render rich image/document preview card if container exists
        const previewEl = document.getElementById(`${prefix}-image-preview`);
        if (previewEl && dataUrl) {
            previewEl.innerHTML = `
                <div class="mt-2 p-2.5 bg-slate-900/90 rounded-xl border border-slate-700 flex items-center justify-between gap-3 shadow-md">
                    <div class="flex items-center gap-3 overflow-hidden cursor-pointer" onclick="window.AttachmentManager && window.AttachmentManager.viewCurrentDropzoneFile('${prefix}')" title="คลิกดูไฟล์เต็ม">
                        ${meta.type === 'image' ? `
                            <img src="${dataUrl}" class="w-12 h-12 object-cover rounded-lg border border-amber-500/50 shrink-0 hover:opacity-90" />
                        ` : `
                            <div class="w-10 h-10 rounded-lg ${meta.bgClass} border ${meta.borderClass} flex items-center justify-center text-lg ${meta.colorClass} shrink-0">
                                <i class="${meta.icon}"></i>
                            </div>
                        `}
                        <div class="truncate text-left">
                            <div class="text-xs font-bold text-white truncate hover:underline">${fileName || 'ไฟล์แนบ'}</div>
                            <div class="text-[11px] text-slate-400 font-mono mt-0.5">${meta.label} • ${fileSizeStr || 'พร้อมใช้งาน'}</div>
                            <div class="text-[10px] text-emerald-400 mt-0.5 flex items-center gap-1">
                                <i class="fa-solid fa-circle-check text-[9px]"></i> พร้อมบันทึกลงฐานข้อมูล (คลิกดูขยาย)
                            </div>
                        </div>
                    </div>
                    <div class="flex items-center gap-1.5 shrink-0">
                        <button type="button" class="btn btn-sm btn-outline text-blue-400 hover:text-white px-2 py-1 text-xs" onclick="window.AttachmentManager && window.AttachmentManager.viewCurrentDropzoneFile('${prefix}')" title="ดูไฟล์เต็ม">
                            <i class="fa-solid fa-eye"></i>
                        </button>
                        <button type="button" class="btn btn-sm btn-outline text-rose-400 hover:text-white px-2 py-1 text-xs" onclick="window.AttachmentManager && window.AttachmentManager.resetDropzone('${prefix}')" title="ลบไฟล์นี้">
    ...
    setDropzoneValue(prefix, val, nameHint = null) {
        const config = window._universalDropzones && window._universalDropzones[prefix];
        ...
        let cleanVal = (typeof val === 'string') ? val.trim() : '';
        if (cleanVal.startsWith('[') && cleanVal.endsWith(']')) {
            try {
                const parsed = JSON.parse(cleanVal);
                if (Array.isArray(parsed) && parsed.length > 0) {
                    const first = parsed[0];
                    cleanVal = typeof first === 'object' ? (first.data || '') : String(first);
                }
            } catch (e) {
                const match = cleanVal.match(/(data:[^"\']+|https?:\/\/[^"\']+)/);
        ...
        if (!fileName) {
            if (cleanVal.startsWith('http')) {
                fileName = cleanVal.split('/').pop().split('?')[0] || 'ไฟล์แนบภายนอก';
            } else if (cleanVal.startsWith('data:image')) {
                fileName = 'ภาพถ่ายที่แนบ.jpg';
            } else if (cleanVal.startsWith('data:application/pdf')) {
                fileName = 'เอกสารแนบ.pdf';
            } else if (cleanVal.startsWith('data:application/vnd') || cleanVal.startsWith('data:application/msword')) {
                fileName = 'เอกสารแนบ.docx';
            } else {
                fileName = 'ไฟล์แนบ';
            }
        }

        const meta = this.detectFileTypeMeta(fileName || cleanVal);
        const approxSize = cleanVal.startsWith('data:') ? this.formatFileSize(Math.round((cleanVal.length * 3) / 4)) : 'ลิงก์ภายนอก';
        ...
        if (config) {
            ...
            this.updateDropzoneUI(prefix, fileName, approxSize, cleanVal);
            if (config.urlInput && config.urlInput.value !== cleanVal) {
                config.urlInput.value = cleanVal;
            }
            if (typeof config.onFileChange === 'function') {
                config.onFileChange(config.currentFile);
            }
        }
    }

    resetDropzone(prefix, clearUrlInput = true) {
        const config = window._universalDropzones && window._universalDropzones[prefix];
        if (config) {
            config.currentFile = null;
            if (config.fileInput) config.fileInput.value = '';
            if (config.promptEl) config.promptEl.classList.remove('hidden');
            if (config.infoEl) {
                config.infoEl.classList.add('hidden');
                config.infoEl.classList.remove('flex');
            }
            if (clearUrlInput && config.urlInput) {
                config.urlInput.value = '';
            }
            const previewEl = document.getElementById(`${prefix}-image-preview`);
            if (previewEl) previewEl.innerHTML = '';
            if (typeof config.onFileChange === 'function') {
                config.onFileChange(null);
            }
        }
    }

    viewCurrentDropzoneFile(prefix) {
        const config = window._universalDropzones && window._universalDropzones[prefix];
        if (!config || !config.currentFile) {
            // Check urlInput value as fallback
            const url = config && config.urlInput && config.urlInput.value;
            if (url) {
                this.openMediaViewer({ data: url, name: 'ไฟล์แนบ' });
            }
            return;
        }
        this.openMediaViewer(config.currentFile);
    }
    initAllDropzones() {
        const prefixes = ['inf', 'elec', 'pwq', 'qwq', 'mach', 'maint', 'risk', 'inc', 'eq', 'drep', 'man', 'user'];
        prefixes.forEach(p => {
            try {
                this.bindUniversalDropzone(p);
            } catch (err) {
                console.warn(`Error initializing dropzone ${p}:`, err);
            }
        });
    }
}
...
// Automatically initialize all dropzones when DOM is ready
if (typeof document !== 'undefined') {
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', () => {
            if (window.AttachmentManager && typeof window.AttachmentManager.initAllDropzones === 'function') {
                window.AttachmentManager.initAllDropzones();
            }
        });
    } else {
        window.AttachmentManager.initAllDropzones();
