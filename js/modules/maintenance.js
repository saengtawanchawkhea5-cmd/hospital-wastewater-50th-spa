...
class MaintenanceModule {
    ...
    bindEvents() {
        ...
        const btnAddSample = document.getElementById('btn-add-maintenance-sample');
        if (btnAddSample) btnAddSample.addEventListener('click', () => this.quickAddPMRecord());
        ...
        // เมื่อเลือกอุปกรณ์ ให้ดึงสถานที่ตั้งจาก data-location มาใส่ให้อัตโนมัติ
        const selMaintEq = document.getElementById('maint-form-equipment');
        if (selMaintEq) {
            selMaintEq.addEventListener('change', (e) => {
                const opt = e.target.selectedOptions?.[0];
                if (opt && opt.dataset && opt.dataset.location) {
                    const locInput = document.getElementById('maint-form-location');
                    if (locInput) {
    ...
    async quickAddNewEquipment() {
                ...
                <div class="space-y-3 text-left">
                    ...
                    <div>
                        ...
                        <select id="swal-maint-input-eq-cat" class="swal2-select !mt-0 !w-full text-xs">
                            ...
                            <option value="เครื่องสูบน้ำเสีย">เครื่องสูบน้ำเสีย (Pumps)</option>
                            <option value="เครื่องเติมอากาศ">เครื่องเติมอากาศ (Blowers)</option>
                            <option value="เครื่องจ่ายสารเคมี">เครื่องจ่ายสารเคมี (Dosing Pumps)</option>
                            <option value="เครื่องมือวัดและเซนเซอร์">เครื่องมือวัดและเซนเซอร์ (Instruments)</option>
                    ...
                    <div>
                        ...
                        <input id="swal-maint-input-eq-loc" class="swal2-input !mt-0 !w-full text-xs" placeholder="เช่น อาคารระบบบำบัดน้ำเสีย, บ่อพักน้ำเสียรวม">
                    </div>
                </div>
            `,
            focusConfirm: false,
        ...
        if (formValues) {
            try {
                const newRecord = {
                    id: 'eq-' + Date.now(),
                    equipment_code: 'EQ-' + String(Date.now()).slice(-4),
                ...
                await this.populateEquipmentSelect(formValues.equipment_name);
                const sel = document.getElementById('maint-form-equipment');
                if (sel) sel.value = formValues.equipment_name;
                const locInput = document.getElementById('maint-form-location');
                if (locInput && formValues.location) locInput.value = formValues.location;

                Swal.fire({
                    icon: 'success',
                    title: 'เพิ่มอุปกรณ์ใหม่เรียบร้อย',
                    text: `เพิ่ม "${formValues.equipment_name}" เข้าสู่ฐานข้อมูล Equipment Reference แล้ว`,
                    timer: 1500,
                    showConfirmButton: false
                });
            } catch (err) {
    ...
    applyFilters() {
        this.filteredItems = this.items.filter(item => {
            if (this.filters.search) {
                ...
                const eqArr = Array.isArray(item.equipment_list) && item.equipment_list.length > 0 ? item.equipment_list : (item.equipment_name ? [item.equipment_name] : []);
                const matchEq = eqArr.join(' ').toLowerCase().includes(q);
    ...
    renderTable(list) {
        ...
        tbody.innerHTML = pageItems.map((item, idx) => {
            ...
            const eqList = Array.isArray(item.equipment_list) && item.equipment_list.length > 0 ? item.equipment_list : (item.equipment_name ? [item.equipment_name] : []);
    ...
    async openAddModal() {
        ...
        // รีเซ็ตดรอปดาวน์อุปกรณ์
        const selEq = document.getElementById('maint-form-equipment');
        if (selEq) selEq.value = '';

        // เปิด Modal ก่อนทันที เพื่อความรวดเร็วและตอบสนองทันใจ
        window.App.openModal('modal-maintenance');
        await this.populateEquipmentSelect();
    }

    /**
     * สุ่มตัวอย่างงานบำรุงรักษาเชิงป้องกัน (PM) สมจริงลงตารางงานซ่อมบำรุงโดยตรง
     * ไม่เปิดโมดอลน้ำเสีย และบันทึกลง Supabase Cloud พร้อมแสดงผลทันที
     */
    async quickAddPMRecord() {
        const pmTemplates = [
            {
                equipment: 'เครื่องเติมอากาศ Roots Blower #1',
                problem: 'ถึงรอบบำรุงรักษาเชิงป้องกัน (PM) ประจำเดือน',
                cause: 'การเสื่อมสภาพตามชั่วโมงการทำงาน (Run time > 720 ชม.)',
                method: 'เปลี่ยนถ่ายน้ำมันหล่อลื่นเกียร์และแบริ่ง, ตรวจสอบความตึงสายพาน V-Belt, ทำความสะอาดไส้กรองอากาศทางดูด',
                result: 'เครื่องเดินเงียบ อุณหภูมิผิวเครื่องปกติ 42°C แรงดันลม 0.45 bar อัตราการไหลปกติ',
                cost: 1500,
                location: 'อาคารระบบบำบัดน้ำเสีย - ห้องเครื่องเป่าลม'
            },
            {
                equipment: 'ปั๊มจุ่มสูบน้ำเสีย Submersible Pump #1',
                problem: 'ถึงรอบตรวจสอบบำรุงรักษาเชิงป้องกัน (PM) ประจำไตรมาส',
                cause: 'การสะสมของคราบไขมันและตะกอนในตัวเรือนปั๊ม',
                method: 'ยกปั๊มตรวจเช็ค Mechanical Seal, ล้างทำความสะอาดใบพัดและห้องสูบ, วัดค่าความต้านทานฉนวนมอเตอร์ (>50 MΩ)',
                result: 'กระแสไฟฟ้าทำงาน 4.8A ปกติ ไม่มีการรั่วซึมของน้ำมันซีล พร้อมใช้งาน',
                cost: 2200,
                location: 'บ่อสูบน้ำเสียรวม (Equalization Basin)'
            },
            {
                equipment: 'ปั๊มจ่ายสารคลอรีน Dosing Pump #1',
                problem: 'ถึงรอบตรวจเช็คระบบท่อส่งและหัวฉีดสารเคมี',
                cause: 'การตกตะกอนของผลึกคลอรีนบริเวณ Diaphragm Valve',
                method: 'ล้างทำความสะอาด Check Valve, เปลี่ยนแผ่นไดอะแฟรมสำรอง, สอบเทียบอัตราการจ่ายสารคลอรีน 15 มล./นาที',
                result: 'ปั๊มจ่ายสารเคมีแม่นยำ ไม่พบรอยรั่วซึมที่ข้อต่อ',
                cost: 850,
                location: 'ห้องเตรียมและจ่ายสารเคมี'
            },
            {
                equipment: 'หัววัดค่าออนไลน์ Online pH / DO Sensor',
                problem: 'ถึงรอบสอบเทียบและทำความสะอาดหัวเซนเซอร์ประจำสัปดาห์',
                cause: 'คราบฟิล์มจุลินทรีย์ (Biofilm) เกาะติดบริเวณกระจกอิเล็กโทรด',
                method: 'ทำความสะอาดหัวโพรบด้วยสารละลายกรดอ่อน, Calibrate ด้วยสารละลายบัฟเฟอร์มาตรฐาน pH 4.01, 7.00 และ 10.01',
                result: 'ค่าอ่านกลับมาตรงตามค่ามาตรฐาน ค่า Slope 98.5% ผ่านเกณฑ์รับรอง',
                cost: 500,
                location: 'ถังตกตะกอน / สระน้ำทิ้ง'
            },
            {
                equipment: 'เครื่องวัดอัตราการไหล Flow Meter',
                problem: 'ถึงรอบตรวจเช็คสภาพสายสัญญาณและ Zero Calibration',
                cause: 'การสะสมของความชื้นและฝุ่นละอองรอบตัวตู้แปลงสัญญาณ Transmitter',
                method: 'เป่าทำความสะอาดตู้คอนโทรล ตรวจสอบขั้วต่อสาย 4-20mA และทำการ Zero Point Calibration',
                result: 'สัญญาณเชื่อมโยงกับระบบบันทึกแม่นยำ ค่าแสดงผลตรงกับอัตราสูบจริง',
                cost: 650,
                location: 'จุดระบายน้ำทิ้งส่วนกลาง'
            }
        ];

        const tpl = pmTemplates[Math.floor(Math.random() * pmTemplates.length)];
        const currentUser = (window.AuthService && typeof window.AuthService.getCurrentUser === 'function')
            ? window.AuthService.getCurrentUser()
            : null;
        const operatorName = currentUser ? (currentUser.full_name || currentUser.username) : "แสงตะวัน ชาวเขา";
        ...
        const newRecord = {
            ...
            job_type: 'ป้องกัน',
            equipment_list: [tpl.equipment],
            problem: tpl.problem,
            cause: tpl.cause,
            fix_method: tpl.method,
            fix_result: tpl.result,
            cost: tpl.cost,
            operator: operatorName,
            status: 'เสร็จสิ้น',
            remarks: 'บันทึกผ่านระบบสุ่มตัวอย่างงานบำรุงรักษาเชิงป้องกัน (PM) อัตโนมัติ',
            created_at: isoStr,
            updated_at: isoStr
        };

        try {
            await window.DataStore.insert('maintenance_records', newRecord);
            await this.loadData();

            if (window.Swal) {
                Swal.fire({
                    icon: 'success',
                    title: 'บันทึกงานซ่อม PM เรียบร้อย!',
                    html: `
                        <div class="text-left text-xs space-y-1.5 p-2 font-sans">
                            <div><strong class="text-amber-400">อุปกรณ์:</strong> ${tpl.equipment}</div>
                            <div><strong class="text-amber-400">รายการ:</strong> ${tpl.problem}</div>
                            <div><strong class="text-emerald-400">ผลการดำเนินการ:</strong> ${tpl.result}</div>
                            <div><strong class="text-cyan-400">ค่าใช้จ่าย:</strong> ฿${tpl.cost.toLocaleString('th-TH', {minimumFractionDigits: 2})}</div>
                            <div><strong class="text-slate-300">ผู้บันทึก:</strong> ${operatorName}</div>
                        </div>
                    `,
                    timer: 2500,
                    showConfirmButton: false,
            ...
            console.error('Failed to quick add PM record:', err);
            if (window.Swal) {
                Swal.fire({
                    icon: 'error',
    ...
    async openEditModal(id) {
        ...
        window.App.openModal('modal-maintenance');
        const existingEq = (Array.isArray(item.equipment_list) && item.equipment_list[0]) || item.equipment_name || '';
        await this.populateEquipmentSelect(existingEq);
        const selEq = document.getElementById('maint-form-equipment');
        if (selEq && existingEq) {
            selEq.value = existingEq;
        }
    }

    async populateEquipmentSelect(selectedVal = null) {
        const sel = document.getElementById('maint-form-equipment');
        if (!sel) return;

        let refList = [];
        try {
            refList = (await window.DataStore.getAll('equipment_ref', { orderBy: 'category', ascending: true })) || [];
        } catch (e) {
            console.warn('Failed to load equipment_ref for maintenance modal:', e);
        }
        ...
        // จัดกลุ่มตามหมวดหมู่อุปกรณ์
        const grouped = {};
        (refList || []).forEach(eq => {
            const cat = eq.category || 'เครื่องจักรและอุปกรณ์ทั่วไป';
            if (!grouped[cat]) grouped[cat] = [];
            grouped[cat].push(eq);
        });

        let html = '<option value="">-- กรุณาเลือกอุปกรณ์ / เครื่องจักรจากฐานข้อมูลอ้างอิง --</option>';
        const sortedCats = Object.keys(grouped).sort((a, b) => a.localeCompare(b, 'th'));

        sortedCats.forEach(cat => {
            html += `<optgroup label="📂 หมวด: ${cat}">`;
            grouped[cat].sort((a, b) => (a.equipment_name || '').localeCompare(b.equipment_name || '', 'th'));
            grouped[cat].forEach(eq => {
                const codePrefix = eq.equipment_code ? `[${eq.equipment_code}] ` : '';
                const locSuffix = eq.location ? ` - ${eq.location}` : '';
                const val = eq.equipment_name;
                const isSel = selectedVal && (selectedVal === val || selectedVal === eq.id || selectedVal === eq.equipment_code) ? 'selected' : '';
                html += `<option value="${val}" data-location="${eq.location || ''}" data-code="${eq.equipment_code || ''}" data-cat="${eq.category || ''}" ${isSel}>${codePrefix}${eq.equipment_name}${locSuffix}</option>`;
            });
            html += `</optgroup>`;
        });

        // หากมีค่า selectedVal เดิมแต่ไม่ตรงกับใน refList ให้เติมเข้าไปเพื่อไม่ให้ข้อมูลสูญหาย
        if (selectedVal && !refList.some(e => e.equipment_name === selectedVal || e.equipment_code === selectedVal)) {
            html += `<optgroup label="📌 อุปกรณ์ที่ระบุไว้เดิม">`;
    ...
    viewDetails(id) {
        ...
        const eqList = Array.isArray(item.equipment_list) && item.equipment_list.length > 0 
            ? item.equipment_list 
    ...
    async saveData() {
        ...
        const selEqEl = document.getElementById('maint-form-equipment');
        const selectedEquipment = selEqEl ? selEqEl.value.trim() : '';

        if (!selectedEquipment) {
            if (window.Swal) {
                Swal.fire({
                    icon: 'warning',
                    title: 'กรุณาเลือกอุปกรณ์ที่เกี่ยวข้อง',
                    text: 'โปรดเลือกอุปกรณ์หรือเครื่องจักรจากฐานข้อมูลอ้างอิง หรือกด "+ เพิ่มอุปกรณ์ใหม่"',
                    confirmButtonColor: '#f59e0b'
                });
            }
            if (selEqEl) selEqEl.focus();
            return;
        }
    ...
    exportExcel() {
        const data = this.filteredItems.map(item => ({
            ...
            'อุปกรณ์': Array.isArray(item.equipment_list) && item.equipment_list.length > 0 ? item.equipment_list.join(', ') : (item.equipment_name || ''),
