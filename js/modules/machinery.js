...
class MachineryModule {
    ...
    bindEvents() {
        ...
        // ผูก Universal Dropzone สำหรับระบบเครื่องจักร (รองรับ อัปโหลดรูปภาพ, PDF, เอกสาร, URL, Base64)
        if (window.AttachmentManager) {
            window.AttachmentManager.bindUniversalDropzone('mach', {
                onFileChange: (file) => {
                    if (file) {
        ...
        // ฟังเหตุการณ์เมื่อมีการกรอก/วางค่าใน URL/Base64 input โดยตรง
        const machImgInput = document.getElementById('mach-form-image');
        if (machImgInput) {
            machImgInput.addEventListener('input', () => {
                const val = (machImgInput.value || '').trim();
                if (val && window.AttachmentManager) {
                    window.AttachmentManager.setDropzoneValue('mach', val);
                }
            });
        }
    ...
    async openAddModal() {
        ...
        this.uploadedImages = [];
        if (window.AttachmentManager) {
            window.AttachmentManager.resetDropzone('mach');
        } else {
            this.renderImageGallery();
        }
        const machImgInput = document.getElementById('mach-form-image');
        if (machImgInput) machImgInput.value = '';
        const machImgPrev = document.getElementById('mach-image-preview');
        if (machImgPrev) machImgPrev.innerHTML = '';
    ...
    async openEditModal(id) {
        const item = this.items.find(x => String(x.id) === String(id)) || this.filteredItems.find(x => String(x.id) === String(id));
        if (!item) return;
        ...
        if (window.AttachmentManager) {
            if (item.image_url) {
                window.AttachmentManager.setDropzoneValue('mach', item.image_url);
            } else {
                window.AttachmentManager.resetDropzone('mach');
            }
        } else {
        ...
        const machImgInput = document.getElementById('mach-form-image');
        if (machImgInput) machImgInput.value = item.image_url || '';
        ...
        Swal.fire({
            ...
            html: `
                <div class="text-left text-xs space-y-3.5 p-2 sm:p-3.5 bg-slate-900/95 rounded-2xl border border-slate-800 text-slate-200">
                    ...
                    <!-- 1. Equipment Checklist (Full Width) -->
                    <div class="p-3 bg-slate-950/60 rounded-xl border border-slate-800/80">
                        <div class="flex items-center justify-between mb-2 pb-1.5 border-b border-slate-800">
                            <strong class="text-cyan-400 flex items-center gap-1.5 font-bold">
                                <i class="fa-solid fa-circle-check text-cyan-400"></i> รายการอุปกรณ์ที่ตรวจเช็ค (${eqList.length} รายการ):
                            </strong>
                            <span class="text-[11px] text-emerald-400 font-mono font-bold"><i class="fa-solid fa-check-double mr-1"></i>ตรวจเช็คสมบูรณ์</span>
                        </div>
                        <div class="flex flex-wrap gap-1.5 max-h-48 overflow-y-auto pr-1">
                            ${eqList.length > 0 ? eqList.map(e => {
                                const isDaySpecific = e.includes('เมนต์โฮลด์') || e.includes('แมนโฮลด์') || e.includes('บ่อออนไซน์');
                                if (isDaySpecific) {
                                    return `<span class="badge bg-amber-950/70 text-amber-300 border border-amber-800/70 text-[11px] py-1 px-2.5"><i class="fa-solid fa-calendar-check text-[9px] mr-1 text-amber-400"></i>${e}</span>`;
                                }
                                return `<span class="badge bg-slate-800/90 text-slate-200 border border-slate-700/70 text-[11px] py-1 px-2.5">${e}</span>`;
                            }).join('') : '<span class="text-slate-500 text-xs py-1">ไม่มีรายการที่เลือก</span>'}
                        </div>
                    </div>

                    <!-- 3. Defects / Abnormalities Section (Full Width, underneath equipment) -->
                    ${(item.abnormal_equipment || item.cause || item.solution || (item.status && item.status !== 'ปกติทุกรายการ')) ? `
                    <div class="p-3.5 bg-rose-950/40 border border-rose-800/60 rounded-xl space-y-2.5">
                        <div class="flex items-center justify-between mb-1 pb-1 border-b border-rose-900/40">
                            <strong class="text-rose-400 flex items-center gap-1.5 font-bold">
                                <i class="fa-solid fa-triangle-exclamation text-rose-400"></i> ข้อบกพร่องที่พบ (${item.status || 'พบข้อบกพร่อง'}):
                            </strong>
                        </div>
                        ${(() => {
                            const rawEqs = (item.abnormal_equipment || '').split(/[,|]/).map(s => s.trim()).filter(Boolean);
                            const rawCauses = (item.cause || '').split(/[|]/).map(s => s.trim()).filter(Boolean);
                            const rawSols = (item.solution || '').split(/[|]/).map(s => s.trim()).filter(Boolean);
                            const maxLen = Math.max(rawEqs.length, rawCauses.length, rawSols.length, 1);

                            if (maxLen <= 1) {
                                return `
                                    <div class="p-3 bg-slate-900/90 rounded-xl border border-rose-900/50 text-xs space-y-1.5">
                                        <div class="text-rose-300 font-bold"><i class="fa-solid fa-gear text-rose-400 mr-1.5"></i> <strong>อุปกรณ์:</strong> ${item.abnormal_equipment || 'พบปัญหาในการตรวจเช็ค'}</div>
                            ...
                            for (let i = 0; i < maxLen; i++) {
                                rows += `
                                    <div class="p-3 bg-slate-900/90 rounded-xl border border-rose-900/50 text-xs space-y-1.5 mb-2 last:mb-0">
                                        <div class="text-rose-300 font-bold"><i class="fa-solid fa-gear text-rose-400 mr-1.5"></i> รายการที่ ${i + 1}: ${rawEqs[i] || item.abnormal_equipment || '-'}</div>
                        ...
                        if (detailImages.length === 0) {
                            return `
                                <div class="pt-2 border-t border-slate-800">
                                    <div class="p-3 bg-slate-950/50 rounded-xl border border-slate-800 text-slate-500 text-xs text-center flex items-center justify-center gap-2">
                                        <i class="fa-regular fa-image text-slate-600"></i>
                                        <span>ไม่มีรูปภาพหรือไฟล์เอกสารแนบในรอบนี้</span>
                                    </div>
                                </div>
                            `;
                        }
                        return `
                            <div class="pt-2 border-t border-slate-800">
                                <div class="flex items-center justify-between mb-2">
                                    ...
                                    <span class="text-[11px] text-slate-400">คลิกเพื่อดูภาพขยาย / เปิดดูเอกสาร</span>
                                </div>
                                <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2.5">
                                    ${detailImages.map((att, i) => {
                                        const meta = window.AttachmentManager ? window.AttachmentManager.detectFileTypeMeta(att.name || att.data, att.type) : { type: 'doc', label: 'เอกสาร', icon: 'fa-solid fa-file-lines', colorClass: 'text-cyan-400', bgClass: 'bg-cyan-500/20', borderClass: 'border-cyan-500/40' };
                                        
                                        if (att.type === 'image' || meta.type === 'image') {
                                            return `
                                                <div class="relative rounded-xl overflow-hidden border border-slate-700/80 bg-slate-950 group cursor-pointer hover:border-purple-500/70 transition-all shadow-md" 
                                                     onclick="window.MachineryModule.viewDetailAttachment('${item.id}', ${i})" title="คลิกดูภาพขยาย">
                                                    ...
                                                    <img src="${att.data}" class="w-full h-32 object-cover group-hover:scale-105 transition-transform duration-200" alt="${att.name || 'รูปภาพ'}" />
                                                    <div class="p-2 bg-slate-900/90 text-[11px] text-slate-300 truncate border-t border-slate-800">
                                                        <i class="fa-solid fa-camera text-amber-400 mr-1"></i> ${att.name || 'ภาพถ่ายเครื่องจักร'}
                                                    </div>
                                                </div>
                                            `;
                                        }
                                        ...
                                        return `
                                            <div class="p-3 rounded-xl bg-slate-950 border ${meta.borderClass} flex flex-col justify-between cursor-pointer hover:border-emerald-400 transition-all shadow-md group" onclick="window.MachineryModule.viewDetailAttachment('${item.id}', ${i})" title="คลิกเปิดไฟล์">
                                                <div class="flex items-start gap-3">
                                                    <div class="w-10 h-10 rounded-lg ${meta.bgClass} border ${meta.borderClass} flex items-center justify-center text-lg ${meta.colorClass} shrink-0 group-hover:scale-105 transition-transform">
                                                        <i class="${meta.icon}"></i>
                                                    </div>
                                                    <div class="truncate flex-1">
                                                        <div class="text-xs font-bold text-white truncate group-hover:text-emerald-300">${att.name || 'เอกสารแนบ'}</div>
                                                        <div class="text-[10px] text-slate-400 font-mono mt-0.5">${meta.label} • ${att.size || 'พร้อมใช้งาน'}</div>
                                                    </div>
                                                </div>
                                                <div class="mt-2.5 pt-2 border-t border-slate-800 flex items-center justify-between text-[10px] ${meta.colorClass}">
                                                    <span><i class="fa-solid fa-arrow-up-right-from-square mr-1"></i>คลิกเพื่อเปิดดูไฟล์</span>
    ...
    viewDetailAttachment(id, index) {
        const item = this.items.find(x => String(x.id) === String(id)) || this.filteredItems.find(x => String(x.id) === String(id));
        if (!item || !item.image_url) return;
        const detailImages = window.AttachmentManager 
            ? window.AttachmentManager.normalizeAttachments(item.image_url) 
            : [];
        const att = detailImages[index];
        if (att && window.AttachmentManager) {
            window.AttachmentManager.openMediaViewer(att);
        }
    }
    ...
    async saveData() {
        ...
        // ประมวลผลรูปภาพและเอกสารแนบ (Universal Attachment Manager & Dropzone)
        let finalImageUrl = null;
        const dropzoneConfig = window._universalDropzones && window._universalDropzones['mach'];
        const dropzoneFile = dropzoneConfig ? dropzoneConfig.currentFile : null;
        const machFormImg = document.getElementById('mach-form-image')?.value?.trim();

        if (this.uploadedImages && this.uploadedImages.length > 0) {
            finalImageUrl = window.AttachmentManager 
                ? window.AttachmentManager.serializeAttachments(this.uploadedImages, false)
                : JSON.stringify(this.uploadedImages);
        } else if (dropzoneFile && dropzoneFile.data) {
            const fileItem = {
                id: 'img_' + Date.now(),
                name: dropzoneFile.name || 'ไฟล์แนบ',
                size: dropzoneFile.size || '-',
                type: dropzoneFile.category || 'image',
                data: dropzoneFile.data
            };
            finalImageUrl = window.AttachmentManager
                ? window.AttachmentManager.serializeAttachments([fileItem], false)
                : JSON.stringify([fileItem]);
        } else if (machFormImg) {
            finalImageUrl = machFormImg;
        }
        ...
        try {
            ...
            // อัปเดตรายการในหน่วยความจำทันทีเพื่อให้ตารางและสถิติเปลี่ยนทันที
            const savedItem = (res && res.record) ? res.record : { ...payload, id: this.editingId || ('temp_' + Date.now()) };
            if (this.editingId) {
                const eIdx = this.items.findIndex(x => String(x.id) === String(this.editingId));
                if (eIdx >= 0) this.items[eIdx] = savedItem;
            } else {
                this.items.unshift(savedItem);
            }
            this.applyFilters();

            // โหลดข้อมูลล่าสุดจากฐานข้อมูลเพื่อความสมบูรณ์
            await this.loadData();
    ...
    previewData() {
        ...
        // 1. คำนวณค่าทางสถิติของเครื่องจักรจากข้อมูลจริงในฐานข้อมูล
        ...
        const normalPercent = totalRounds > 0 ? ((normalRounds / totalRounds) * 100).toFixed(1) : '0.0';
        ...
        const readinessPercent = totalRounds > 0 ? (((totalRounds - abnormalItemsTotal) / totalRounds) * 100).toFixed(1) : '0.0';

        // 2. สรุป 10 อันดับอุปกรณ์/หมวดหมู่ที่ได้รับการตรวจสอบสูงสุด (ดึงจากฐานข้อมูลจริงเท่านั้น)
        const eqFreqMap = {};
        ...
        const sortedEqs = Object.entries(eqFreqMap).sort((a, b) => b[1] - a[1]);
        ...
        if (top10.length === 0) {
            top10RowsHtml = `
                <tr>
                    <td colspan="5" style="text-align: center; color: #94a3b8; padding: 18px; font-size: 12px;">
                        <i class="fa-solid fa-inbox" style="font-size: 22px; margin-bottom: 6px; display: block; color: #cbd5e1;"></i>
                        ไม่มีข้อมูลการตรวจเช็คเครื่องจักรในฐานข้อมูล
                    </td>
                </tr>
            `;
        } else {
            top10RowsHtml = top10.map(([name, count], index) => {
                const percent = totalEqChecks > 0 ? ((count / totalEqChecks) * 100).toFixed(1) : '0.0';
        ...
        // 3. รายการบันทึกการตรวจสอบล่าสุด (20 รายการล่าสุด) พร้อมคอลัมน์รูปภาพ / เอกสารแนบ
        ...
        const recentRowsHtml = recentSlice.map((item, index) => {
            ...
            const attachments = window.AttachmentManager 
                ? window.AttachmentManager.normalizeAttachments(item.image_url) 
                : [];
            let attachHtml = '<span style="color: #94a3b8; font-size: 10px;">-</span>';
                ...
                if (isImg) {
                    attachHtml = `
                        <div style="display: inline-flex; align-items: center; gap: 4px;">
                            <img src="${first.data}" alt="รูปภาพ" style="width: 30px; height: 30px; object-fit: cover; border-radius: 4px; border: 1px solid #cbd5e1; cursor: pointer;" onclick="window.MachineryModule.viewDetails('${item.id}')" title="คลิกเพื่อดูรูปภาพขยาย (${attachments.length} รูป)" />
                            ${attachments.length > 1 ? `<span style="font-size: 9px; color: #7c3aed; font-weight: bold; background: #f3e8ff; border: 1px solid #e9d5ff; border-radius: 4px; padding: 1px 4px;">+${attachments.length - 1}</span>` : ''}
                        </div>
                    `;
                } else {
                    attachHtml = `
                        <div style="display: inline-flex; align-items: center; gap: 4px; cursor: pointer;" onclick="window.MachineryModule.viewDetails('${item.id}')" title="คลิกเพื่อดูเอกสาร">
                            <span style="font-size: 10px; color: #2563eb; font-weight: 600; background: #eff6ff; border: 1px solid #bfdbfe; border-radius: 4px; padding: 2px 6px;">
                                <i class="fa-solid ${first.type === 'pdf' ? 'fa-file-pdf text-rose-500' : 'fa-file-lines text-blue-500'}"></i> ${first.name || 'เอกสาร'}
                            </span>
                        </div>
                    `;
                }
            ...
            return `
                <tr style="border-bottom: 1px solid #f1f5f9; font-size: 11px;">
                    ...
                    <td style="color: #64748b; font-size: 10px; padding: 6px 10px; max-width: 180px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">
                        ${item.remarks || '-'}
                    </td>
                </tr>
            `;
        ...
        // รวบรวมรูปภาพและเอกสารทั้งหมดเพื่อสร้างคลังภาพถ่ายและเอกสารหลักฐาน (Evidence Gallery)
        const galleryItems = [];
        this.filteredItems.forEach(item => {
            const atts = window.AttachmentManager 
                ? window.AttachmentManager.normalizeAttachments(item.image_url) 
                : [];
            atts.forEach(att => {
                galleryItems.push({ item, attachment: att });
            });
        });
        ...
        if (galleryItems.length > 0) {
            gallerySectionHtml = `
                <!-- Section 3: คลังภาพถ่ายและเอกสารหลักฐานการตรวจเช็ค (Inspection Evidence Gallery) -->
                <div class="mt-5">
                    <div class="text-xs font-bold text-slate-800 mb-2 flex items-center justify-between">
                        <div class="flex items-center gap-1.5">
                            ...
                            <span>คลังภาพถ่ายและเอกสารหลักฐานการตรวจเช็ค (${galleryItems.length} รายการ)</span>
                        </div>
                        <span class="text-[10px] text-slate-500 font-normal">คลิกรูปเพื่อเปิดดูแบบขยายเต็มจอ</span>
                    </div>
                    <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(130px, 1fr)); gap: 10px;" class="p-3 bg-slate-50 border border-slate-200 rounded-lg">
                        ${galleryItems.slice(0, 12).map(({ item, attachment }, gIdx) => {
                            const isImg = attachment.type === 'image' || (!attachment.type && (attachment.data?.startsWith('data:image') || attachment.data?.match(/\.(jpg|jpeg|png|webp|gif)/i)));
                            const dateStr = window.App.formatDateTime(item.recorded_at);
                            const eqFirst = (this.normalizeEquipmentList(item.equipment_list)[0] || 'ตรวจเช็คเครื่องจักร');
                            return `
                                <div style="border: 1px solid #e2e8f0; border-radius: 6px; overflow: hidden; background: #ffffff; box-shadow: 0 1px 2px rgba(0,0,0,0.05); text-align: center;" class="group">
                                    <div style="height: 85px; overflow: hidden; display: flex; align-items: center; justify-content: center; background: #f8fafc; cursor: pointer;" onclick="window.AttachmentManager && window.AttachmentManager.openMediaViewer(${JSON.stringify(attachment).replace(/"/g, '&quot;')})">
                                        ${isImg ? `
                                            <img src="${attachment.data}" alt="${attachment.name || 'รูปภาพ'}" style="width: 100%; height: 100%; object-fit: cover; transition: transform 0.2s;" onmouseover="this.style.transform='scale(1.05)'" onmouseout="this.style.transform='scale(1)'" />
                                        ` : `
                                            <div style="padding: 10px; color: #64748b;">
                                                <i class="fa-solid ${attachment.type === 'pdf' ? 'fa-file-pdf text-rose-500' : 'fa-file-lines text-blue-500'} text-3xl"></i>
                                                <div style="font-size: 9px; font-weight: bold; margin-top: 4px; max-width: 100px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">${attachment.name || 'เอกสาร'}</div>
                                            </div>
                                        `}
                                    </div>
                                    <div style="padding: 4px 6px; text-align: left; border-top: 1px solid #f1f5f9;">
                                        <div style="font-size: 9px; font-weight: 700; color: #1e293b; max-width: 110px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;" title="${eqFirst}">
                                            ${eqFirst}
                                        </div>
            ...
            gallerySectionHtml = `
                <div class="mt-5">
                    <div class="text-xs font-bold text-slate-800 mb-2 flex items-center gap-1.5">
                        <i class="fa-solid fa-images text-purple-600"></i>
                        <span>คลังภาพถ่ายและเอกสารหลักฐานการตรวจเช็ค</span>
                    </div>
                    <div class="p-6 text-center text-slate-400 bg-slate-50 border border-dashed border-slate-300 rounded-lg text-xs">
                        <i class="fa-solid fa-image text-2xl mb-1 text-slate-300 block"></i>
                        ยังไม่มีรูปภาพหรือเอกสารแนบในระบบ
                    </div>
                </div>
            `;
        }
        ...
        sheet.innerHTML = `
            ...
            <!-- Section 2: รายการบันทึกการตรวจสอบล่าสุด (20 รายการ) พร้อมคอลัมน์รูปภาพ -->
            <div>
                <div class="text-xs font-bold text-slate-800 mb-2 flex items-center gap-1.5">
                    ...
                    <table class="doc-table-clean">
                        <thead>
                            <tr>
                                ...
                                <th style="text-align: center; width: 85px;">รูปภาพ/เอกสาร</th>
                                <th>ผู้ตรวจเช็ค</th>
                                <th>หมายเหตุ / สิ่งที่พบ</th>
                            </tr>
                        </thead>
                        <tbody>
                            ${recentRowsHtml || '<tr><td colspan="7" class="text-center py-4 text-slate-400">ไม่พบข้อมูลการตรวจเช็ค</td></tr>'}
                        </tbody>
                    </table>
                </div>
            </div>
