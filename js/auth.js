...
class AuthService {
    ...
    init() {
        // ตรวจสอบเซสชันเฉพาะใน sessionStorage (หมดอายุเมื่อปิดเบราว์เซอร์ เพื่อความปลอดภัย)
        let saved = sessionStorage.getItem(APP_CONFIG.storageKeys.currentUser);
        
        // ล้างเซสชันถาวรใน localStorage ออกเสมอ เพื่อบังคับให้ต้องกรอกรหัสผ่านใหม่ทุกครั้งที่เปิดเข้าสู่ระบบ
        try {
    ...
    saveSession(remember = true) {
        if (this.currentUser) {
            ...
            // บันทึกเฉพาะใน sessionStorage สำหรับการใช้งานในแท็บปัจจุบัน (ปิดเบราว์เซอร์แล้วหมดอายุ)
            sessionStorage.setItem(APP_CONFIG.storageKeys.currentUser, dataStr);

            // ล้างเซสชันถาวรใน localStorage ออกเสมอ เพื่อให้ต้องกรอกรหัสผ่านใหม่ทุกครั้ง
            localStorage.removeItem(APP_CONFIG.storageKeys.currentUser);
            localStorage.removeItem("spa_50th_remember_login");

            // บันทึกเฉพาะ "ชื่อผู้ใช้งาน" (Username) ในเครื่องเท่านั้น
            if (remember && this.currentUser.username) {
                localStorage.setItem("spa_50th_remembered_username", this.currentUser.username);
            } else if (!remember) {
    ...
    getRememberedUsername() {
        return localStorage.getItem("spa_50th_remembered_username") || "";
    }
    ...
    isAdmin() {
        ...
        return role === 'admin' || role === 'superadmin' || username === 'sangtawan';
    }
